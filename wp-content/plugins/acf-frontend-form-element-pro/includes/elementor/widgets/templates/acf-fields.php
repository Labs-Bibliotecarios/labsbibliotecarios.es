...
class ACF_Fields_Widget extends Widget_Base {
	...
	public function show_in_panel(){
		if( isset( $_GET['post'] ) ){
			...
			$current_template = get_post_meta( $post_id, '_elementor_template_type', true );
			if( $current_template == 'post_form' ){
				return true;
			}
		}
		return false;
   ...
   public function get_form_defaults() {
	   return [
			...
			'fields' => [
				...
				[
					...
					'field_label_on' => 'true',
					'field_required' => 'true',
					'post_type_field_options' => array( 'post', 'page' ),
	...
	public function get_keywords() {
		return ['frontend editing', 'edit post', 'add post', 'add user', 'edit user', 'edit site'];
	}
	...
	protected function _register_controls() {
		do_action( 'acff/form_structure_section', $this );
	...
	public function get_field_type_groups(){
		...
		$fields['post']  = [
			...
			'options' => [
				...
				//'parent' => __( 'Parent Post', 'acf-frontend-form-element' ),
				//'post_password' => __( 'Post Password', 'acf-frontend-form-element' ),
	...
	public function get_form_fields( $settings, $wg_id, $form_args = [] ){
				...
				foreach( $groups as $name => $group ){
					if( in_array( $form_field['field_type'], array_keys( $group['options'] ) ) ){
						...
						if( isset( acff()->local_actions[$action_name] ) ){
							$main_action = acff()->local_actions[$action_name];
						}
						break;
					}
				}
			...
			if( isset( $main_action ) ){
				...
				if( isset( $local_field['type'] ) ){
					...
					if( $form_field['field_type'] == 'taxonomy' ){
						...
						$local_field['name'] = $wg_id .  '_' . $form_field['field_type'];$local_field['key'] = $wg_id .  '_' . $form_field['field_type'];
					}
				} 			
			}
	...
	protected function render() {	
		...
		if( $settings['fields_selection'] ){
			$form_fields = $this->get_form_fields( $settings, $wg_id, ['post_id' => $post_id ] );
        }
        $fields = [];
        ...
        ?>
        <div class="acf-fields acf-form-fields -<?php echo esc_attr($settings['field_label_position'])?>">
        <?php acff()->form_display->render_fields( $fields, array( 'post' => $post_id ), $wg_id, 'post', 'div', $settings['field_instruction_position'] ); ?>
        </div>
        <?php
    }

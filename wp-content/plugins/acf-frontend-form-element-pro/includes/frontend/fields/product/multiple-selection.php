...
if( ! class_exists('acf_field_multiple_selection') ) :

class acf_field_multiple_selection extends acf_field {
	...
	*/
	...
	function initialize() {
		...
		$this->label = _x('Multiple Selection', 'noun', 'acf');
		$this->public = false;
		$this->defaults = array(
			'multiple' 		=> 0,
			'allow_null' 	=> 0,
		...
		if( ! empty( $field['sub_fields'] ) ){
            ...
            foreach( $field['sub_fields'] as $sub_field ){
                ...
                if( empty($sub_field['placeholder']) ) {
                    $sub_field['placeholder'] = _x('Select', 'verb', 'acf');
                }
...
// initialize
acf_register_field_type( 'acf_field_multiple_selection' );

...
if( ! class_exists('acf_field_product_length') ) :

class acf_field_product_length extends acf_field_number {
	...
	*/
    ...
    function load_value( $value, $post_id = false, $field = false ){
        $value = get_post_meta( $post_id, '_length', true );
        return $value;
    }
    

    function pre_update_value( $value, $post_id = false, $field = false ){
        update_metadata( 'post', $post_id, '_length', $value );
        return null;
    }
...
// initialize
acf_register_field_type( 'acf_field_product_length' );

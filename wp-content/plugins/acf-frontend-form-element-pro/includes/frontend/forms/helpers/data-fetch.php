...
function acf_frontend_get_field_data( $type = null ){
	...
	if( $acf_field_groups ) {   
		foreach( $acf_field_groups as $field_group ) {
			if( ! empty( $field_group['acff_group'] ) ) continue;
...
function acf_frontend_user_id_fields(){
	$fields = acf_frontend_get_acf_field_choices( false, 'user' );
	$keys = array_merge( ['[author]' => __( 'Post Author', 'acf-frontend-form-element' ) ],  $fields );
	return $keys;
}

function acf_frontend_get_user_roles( $exceptions = [], $all = false ){
...
function acf_frontend_get_user_caps( $exceptions = [], $all = false ){
	$user_caps = array();
...
function acf_frontend_get_acf_field_choices( $groups = false, $type = false ){
...
function acf_frontend_edit_mode(){
	...
	if( ! empty( acff()->elementor ) ) $edit_mode = \Elementor\Plugin::$instance->editor->is_edit_mode();	
	...
	if ( acff()->is__premium_only() ) {
		...
		/* if( $type == 'all' || $type == 'comment' ){
			$fields['comment']  = array(
				...
				'options' => array(
					'comment' => __( 'Comment Body', 'acf-frontend-form-element' ),
					'author' => __( 'Comment Author', 'acf-frontend-form-element' ),
					'author_email' => __( 'Comment Author Email', 'acf-frontend-form-element' ),
				)
			);
		} */
		if( $type == 'all' ){
		...
		if ( class_exists( 'woocommerce' ) ){	
			if( $type == 'all' || $type == 'product' ){
				...
				$fields['product_advanced'] = array(
					'label' => __( 'Product Advanced Options', 'acf-frontend-form-element' ),
					'options' => array(

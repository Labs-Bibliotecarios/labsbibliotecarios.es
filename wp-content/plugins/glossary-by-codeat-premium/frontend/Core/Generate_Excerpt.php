...
class Generate_Excerpt extends Engine\Base {
	...
	public function limit_excerpt( string $excerpt ) {
		...
		if ( 0 !== \absint( $this->settings[ 'excerpt_limit' ] ) ) {
			if ( \strlen( $excerpt ) >= \absint( $this->settings[ 'excerpt_limit' ] ) ) {
				$excerpt_temp = \substr( $excerpt, 0, \absint( $this->settings[ 'excerpt_limit' ] ) ) . $dots;
			}
			...
			if ( !empty( $this->settings[ 'excerpt_words' ] ) ) {
				...
				if ( \strlen( $excerpt ) >= $char_limit ) {
					$excerpt_temp = \wp_trim_words( $excerpt, $char_limit, '' ) . $dots;
				}
			}
		}
